...
@NamedQueries({ @javax.persistence.NamedQuery(name = "Login.DefaultFinders", query = "select e from Login e where e.systemInfo.activeStatus=1 and e.serverAuthImage LIKE :serverAuthImage"), @javax.persistence.NamedQuery(name = "Login.findByUserId", query = "select e from Login e where e.systemInfo.activeStatus=1 and e.user.userId=:userId"), @javax.persistence.NamedQuery(name = "Login.findByContactId", query = "select e from Login e where e.systemInfo.activeStatus=1 and e.coreContacts.contactId=:contactId"), @javax.persistence.NamedQuery(name = "Login.findById", query = "select e from Login e where e.systemInfo.activeStatus=1 and e.loginPk =:loginPk"), @javax.persistence.NamedQuery(name = "Login.FindMappedUser", query = "SELECT u FROM Login u WHERE u.systemInfo.activeStatus=1 AND u.user.userId IN (SELECT ub.userId FROM UserRoleBridge ub)"), @javax.persistence.NamedQuery(name = "Login.FindUnMappedUser", query = "SELECT u FROM Login u WHERE u.systemInfo.activeStatus=1 AND u.user.userId NOT IN (SELECT ub.userId FROM UserRoleBridge ub)") })
